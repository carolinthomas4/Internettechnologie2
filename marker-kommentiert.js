...
//Objektdefinition der Töne in den Reihen (Mapping-Tabelle):
const toneMap = {  
...
//Audio
//Erstellung neues Audio Context Objekts; Schnittstelle zur Erzeugung, Steuerung, Manipulation von Sounds
const audioCtx = new (window.AudioContext || window.webkitAudioContext)();

//Oscillattor Typen (Sinuskurve, Rechteckwellen, Dreieckwelle, sog. Sägezahnozillator)
...
let torusScene, torusCamera, torusRenderer, torusAnimationId; //für 3D muss man Licht und Kamera setzen
let torusMesh;
let torusPhase = 0;
//Row2 Würfel
...
//Formular
//1. Schleife wird 4 Mal (4 Reihen) und 2. Schleife 5 Mal (5 Buttons) durchlaufen
for (let row = 1; row <= 4; row++) { 
	for (let col = 1; col <= 5; col++) {    
		const buttonId = `radioTon${row}${col}`; //erzeugt eindeutige ID für Button    
		const toneKey = toneMap[`row${row}`][col - 1]; //greift auf Mapping Tabelle mit Tönen zu 
		document.getElementById(buttonId).addEventListener('click', () => {      
			startLoop(tones[toneKey].freq, `row${row}`);    
		}); //bei klick auf einen der Radiobuttons wird "start loop" abgerufen
	}
}

//läuft für alle 4 Zeilen durch
for (let row = 1; row <= 4; row++) {  
	const buttonId = `radioTon${row}0`; //Id der Stopp Buttons wird erzeugt (für jede reihe)
	const rowKey = `row${row}`; //Schlüssel für jede zeile "row x" wird erzeugt
	document.getElementById(buttonId).addEventListener('click', () => {   
		stopLoop(rowKey);   
		stopActions[rowKey]();
	}); //bei Klick auf Ton aus Button durchläuft diese Schleife; durchläuft "stop loop"&visuals
}

for (let row = 1; row <= 4; row++) {  
	['volume', 'tempo'].forEach(param => {    
		const elementId = `${param}_row${row}`;//für Volume u. Tempo werden für jede zeile eindeutige IDs erzeugt
		document.getElementById(elementId).addEventListener('input', () => {//Immer wenn regler bewegt wird folgende funktion ausgeführt:   
			const selectedTone = document.querySelector(`input[name="stimmung_row${row}"]:checked`);//ob bzw welcher Ton ausgewählt   
			if (selectedTone) {      
...
//Overlay und Formular ausblenden
...
//Formular inkl Regler zurücksetzen
function resetForm() {
	...
	sliderContainers.forEach(container => {    
		...
		sliders.forEach(slider => {      
			if (slider.name === 'volume') {        
				slider.value = 0.1; //Setzt regler auf anfangswert      
			} else if (slider.name === 'tempo') {        
				slider.value = 700; //Setzt regler auf anfangswert       
			}    
		});  
	});
}
...
function playTone(freq, rowId, duration = 300) {
	...
	osc.type = oscillatorTypes[rowId] || 'sine'; //Auswahl des jew. Oscillator Typen-> Sine als Absicherung, falls er keinen findet
...
function initTorusCanvas() {
	...
	torusCamera.position.z = 200; //Kamera Position
	const light = new THREE.PointLight(0xffffff, 1); //setzt Licht
...
//Row2 Würfel Cubes
function initCubesCanvas() { //Kamera und Licht für 3D setzen
...
function stopAllVisuals(rowId) { //stoppt offensichtlich alle Visuals
